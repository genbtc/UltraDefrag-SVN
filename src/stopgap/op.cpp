  ...
  Operation &op, winx_file_info *f, const winx_volume_region *g)
{
  ...
  while (auto cur = (ULONG)min(numlcns, MAXULONG32 - 1)) {
    NTSTATUS status;
    {
...
void stopgap_move_set(winx_volume_region &r)
{
    ...
    Options opts;
    opts.Reset();
    zen::FileEnumeration::files_t files;
    op.init();
    move_set(op, files, r);
}

void defrag(Operation &op)
{
...
bool widen_behind(Operation &op, const winx_volume_region *g, size_t maxMoves)
{
  auto r = *g;
  size_t moved = 0;
  uint64_t movedlen = 0;
  for (auto i = 0; op.opts.widen && !util::ConsoleHandler::gTerminated &&
...
std::wstring stopgap_enumerate_gaps(char driveLetter)
{
    ...
    verboseOutput << std::setw(20) << "Processing Op.Volume: " << char(toupper(op.opts.volume)) 
        << ": " << op.vol.info.label << " (" << op.vol.info.fs_name << ")" << std::endl;
    verboseOutput << std::setw(20) << "Total size: " << op.vol(op.vol.info.total_clusters) << std::endl;
    verboseOutput << std::setw(20) << "Free size: " << op.vol(op.vol.info.free_bytes / op.vol.info.bytes_per_cluster) << std::endl;
    verboseOutput << std::setw(20) << "Bytes per cluster: " << op.vol.info.bytes_per_cluster << std::endl;
    verboseOutput << std::setw(20) << "Using max gap size: " << op.vol(op.opts.maxSize) << std::endl;
    verboseOutput << std::endl;
    std::wcout << L"Enumerating files…" << std::endl << std::flush;
    std::wcout << verboseOutput.str() << std::endl << std::flush;
    ...
    //Results:
    verboseOutput << "Found " << op.fe->unprocessable() << " unprocessable files." << std::endl;
    verboseOutput << std::endl;
    verboseOutput << "Found " << op.fe->count() << " processable files in total" << std::endl;
...
void Options::parseCmdLine(int argc, wchar_t **argv)
{
  ...
  ("maxsize,m",
   ...
   default_value(MAXIMUM_GAP_SIZE_DEFAULT),
  ...
  ("volume",po::wvalue<std::wstring>(), "Volume to defrag");
...
{  
  ...
  opts.maxSize = static_cast<size_t>(opts.maxSize * 1024 / vol.info.bytes_per_cluster);
...
void Operation::run()
{
  ...
  while (!util::ConsoleHandler::gTerminated && replaced) {
    ...
    if (opts.defrag) {
      // Some defragmentation.
        trace("This is about to defragment stuff on drive");
      defrag(*this);
      ge->scan();
    }
  ...
  std::wcout << L"Carried out " << util::light << moved << util::clear <<
             ...
             util::clear << L" (" << vol(int(movedLen / seconds())) << L"/sec)." <<
             std::endl;
...
std::wstring Operation::metrics() const
{
  ...
  ss << std::fixed << std::setprecision(2) << (moved / s) <<
     L" moves/sec, " << vol(int(movedLen / s)) << L"/sec";
  return ss.str();
}